<template>
  <div>
    ...
    <table class="w-full border" v-if="!loading && roomsData.length">
      ...
      <tbody>
        <tr v-for="room in roomsData" :key="room.id">
          ...
          <td class="border px-2 py-1">{{ room.kapasitas }}</td>
          <td class="border px-2 py-1">{{ room.deskripsi }}</td>
          <td class="border px-2 py-1">
            <span
              :class="room.status === 'aktif' ? 'text-green-600' : 'text-red-600'"
            >
              {{ room.status }}
            </span>
          </td>
    ...
    <!-- Empty state -->
    <p v-if="!loading && roomsData.length === 0" class="text-gray-500">
      No rooms found.
    </p>
...
function openForm(r = null) {
  form.value = r
    ? { ...r }
    : { id: null, name: "", kapasitas: 1, deskripsi: "", status: "aktif" }
  showForm.value = true
}
...
async function save() {
  ...
  if (!form.value.kapasitas || form.value.kapasitas < 1) {
